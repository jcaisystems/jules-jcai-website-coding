// src/components/ui/retro-grid.tsx
"use client";
import { cn } from "@/lib/utils";
import React from "react";

interface RetroGridProps extends React.HTMLAttributes<HTMLDivElement> {
  className?: string;
}

export function RetroGrid({ className, ...props }: RetroGridProps) {
  return (
    <div
      className={cn(
        "pointer-events-none absolute h-full w-full overflow-hidden opacity-50 [perspective:200px]",
        className
      )}
      {...props}
    >
      {/* Grid */}
      <div className="absolute inset-0 [transform:rotateX(65deg)]">
        <div
          className={cn(
            "animate-grid",
            "[background-repeat:repeat] [background-size:60px_60px] [height:300vh] [inset:0%_0px] [margin-left:-50%] [transform-origin:100%_0_0] [width:600vw]",
            // Light-mode grid color
            "[background-image:linear-gradient(to_right,rgba(0,0,0,0.3)_1px,transparent_0),linear-gradient(to_bottom,rgba(0,0,0,0.3)_1px,transparent_0)]",
            // Dark-mode grid color
            "dark:[background-image:linear-gradient(to_right,rgba(203,213,225,0.2)_1px,transparent_0),linear-gradient(to_bottom,rgba(203,213,225,0.2)_1px,transparent_0)]"
          )}
        />
      </div>

      {/* Fading gradient */}
      <div className="absolute inset-0 bg-gradient-to-t from-slate-900 to-transparent to-90%" />
    </div>
  );
}

export default RetroGrid;